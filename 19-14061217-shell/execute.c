...
void execOuterCmd(SimpleCmd *cmd){
    ...
    if(exists(cmd->args[0])){ //命令存在
        ...
        if(pid == 0){ //子进程
            ...
            justArgs(cmd->args[0]);
			setpgid(0,0);//2016年03月24日00:03:18 改
			...
			sleep(1);//2016年03月25日02:23:14
            if(cmd ->isBack){ //后台命令             
                fgPid = 0; //pid置0，为下一命令做准备
                addJob(pid); //增加新的作业
				usleep(1); //2016年03月24日09:36:08改
...
void execSimpleCmd(SimpleCmd *cmd){
    ...
    if(strcmp(cmd->args[0], "exit") == 0) { //exit命令
        ...
        if(history.end == -1){
            //printf("尚未执行任何命令\n");
			printf("there has no command before\n");
            return;
        }
        i = history.start;
        do {
        ...
        if(temp != NULL){
            if(chdir(temp) < 0){
				...
				printf("cd; %s wrong files or floder name\n", temp);
            }
        }
        ...
        if(temp != NULL && temp[0] == '%'){
			...
			printf("fg; illegal parameter, right form:fg %<int>\n");
        }
        ...
        if(temp != NULL && temp[0] == '%'){
			...
			printf("bg; illegal parameter, right form:bg %<int>\n");
        }
    } else if(strcmp(cmd->args[0], "mykill") == 0){// mykill 命令
		temp = cmd->args[1];
		if( temp != NULL){
			pid = str2Pid(temp,0,strlen(temp));
			kill(pid,SIGKILL);
		}
		else{
			printf("mykill; ilegal parameter, right form:mykill <int>\n");
		}
	} else if (strcmp(cmd->args[0], "myecho") == 0){//myecho命令
