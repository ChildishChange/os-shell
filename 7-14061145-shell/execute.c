...
SimpleCmd* handleSimpleCmdStr(int begin, int end){
    ...
    cmd->isBack = 0;
    cmd->input = cmd->output =cmd->progB = NULL;
...
//for(i=0;i<strlen(cmd->progB[0]);i++){
//printf("#%c",cmd->progB[0][i]);
//}
//printf("\n");
    }
...
//printf("即将创建子进程\n");
        ...
        if(pid == 0){ //子进程
//printf("子进程创建成功，子进程pid为 %d ，子进程组id为 %d\n",getpid(),getpgid(0));
...
//printf(SIGUSR1);
            ...
            justArgs(cmd->args[0]);
	    setpgid(pid,pid);
//printf("修改子进程组id，子进程pid为 %d ，子进程组id为 %d\n",getpid(),getpgid(0));
//printf("子进程即将执行命令\n");
...
//printf("我是父进程,父进程pid为 %d ，父进程组id为 %d\n",getpid(),getpgid(0));  
            if(cmd ->isBack){ //后台命令 
//printf("后台命令\n");            
