...
char incommand[9][20]={"fg","exit","history","enable","bg","jobs","cd","type","echo"};
/*******************************************************
                  工具以及辅助方法
********************************************************/
/*判断命令是否存在*/
...
/*type*/
void type_exec(SimpleCmd *cmd)
{
	int i;
	for(i=0;i<8;i++)
	{
		if(strcmp(cmd->args[1],incommand[i])==0)
		{
		printf("Buitin\n");
		return;
		}
	}
	if(exists(cmd->args[1]))
		{printf("OutsideCMD\n");return;}
...
void mySIGCHILD(int sig, siginfo_t *sip, void* noused)
{
...
void init(){
    ...
    action.sa_sigaction = mySIGCHILD;
...
SimpleCmd* handleSimpleCmdStr(int begin, int end){
    ...
    char c, buff[10][40], inputFile[30], outputFile[30], *temp = NULL,outputcmd[30];
    ...
    //if there is output command, providethe output
    if(strlen(outputcmd) != 0){
        j = strlen(outputcmd);
        cmd->ifpipeline = 1;
        cmd->output = (char*)malloc(sizeof(char) * (j + 1));
        strcpy(cmd->output, outputcmd);
    }
    //如果是管道
    if(cmd->ifpipeline==1)
    {
    ...
    if(exists(cmd->args[0])){ //命令存在
	    ...
	    //printf("%d\n",getpgid(0));
            ...
            if(cmd->output != NULL){ //存在输出重定向
                if(cmd->ifpipeline == 0 && (pipeOut = open(cmd->output, O_WRONLY|O_CREAT|O_TRUNC, S_IRUSR|S_IWUSR)) == -1){
                    printf("不能打开文件 %s！\n", cmd->output);
                    return ;
                }
                if(cmd->ifpipeline==0 && dup2(pipeOut, 1) == -1){
                    printf("重定向标准输出错误！\n");
                    return;
                }
            }
...
void execSimpleCmd(SimpleCmd *cmd){
    ...
    }else if(strcmp(cmd->args[0], "enable") == 0){//enable command
            enable_exec();
	} else if(strcmp(cmd->args[0], "type") == 0){//type command
            type_exec(cmd);
	} else if(strcmp(cmd->args[0], "echo") == 0){//echo command
            echo_exec(cmd);
	}
...
int match(char *nameOfInput,char **realName){
    DIR *dp;
    struct dirent *entry;
    char buffer[100];
    int i,j;
    int last_index_file;
    int last_index_input;

    getcwd(buffer, sizeof(buffer));

    if((dp = opendir(&buffer)) == NULL) {//无法打开该目录
        fprintf(stderr,"Sorry,the directory cannot be opened: %s\n", buffer);
        return;
    }
    while((entry = readdir(dp)) != NULL) {
        i=0;
        j=0;
        last_index_input=strlen(nameOfInput)-1;
        last_index_file=strlen(entry->d_name)-1;
        while (i<=last_index_file&&j<=last_index_input){
            if (entry->d_name[i]!=nameOfInput[j])
                break;
            i=i+1;
            j=j+1;
        }
        if (i==strlen(entry->d_name)||nameOfInput[j]!='*'){
            continue;
        }
        else {
            while (last_index_input>=j&&last_index_file>=i){
                if (nameOfInput[last_index_input]!=entry->d_name[last_index_file])
                    break;
                last_index_input--;
                last_index_file--;
            }
            if (last_index_input==j){
                *realName = (char*)malloc(sizeof(char) * (strlen(entry->d_name) + 1));
                strcpy(*realName,entry->d_name);
                return 1;
            }
        }
    }
